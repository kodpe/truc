...
/*   Updated: 2022/11/11 10:07:56 by sloquet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	game_loop(t_m *m)
{
	...
	// smlx_screen_filler(m->mlx.img, 0x1AFF0000);	// light mode
	// update_box(m);
	// receive(m);
	...
	// for (int i = 100; i > 0; i--)
		// smlx_circle_center(m->mlx.img, m->g_center, i, 0x0000FF + i * 100);
...
void	game_room(t_m *m)
{
	...
	// sam.width = 100;
	// sam.height = 100;
	// if (chdir(GDIR_ASSETS))
	// 	abort();
	// dir_log(".");
	// if (smlx_new_xpm(&m->mlx, &sam, "samourai_200.xpm", 0))
	// 	abort();

	// mlx_put_image_to_window(&m->mlx.ptr, m->mlx.win.ptr, &sam, 0, 0);
	getchar();
