...
void	*mlx_new_window_pos(t_xvar *xvar,int size_x,int size_y,char *title, int screen_x, int screen_y)
{
	t_win_list				*new_win;
	XSetWindowAttributes	xswa;
	XGCValues				xgcv;
	...
	*/
	...
	new_win->window = XCreateWindow(xvar->display,xvar->root,screen_x,screen_y,size_x,size_y,
					0,CopyFromParent,InputOutput,xvar->visual,
					CWEventMask|CWBackPixel|CWBorderPixel|
					CWColormap,&xswa);
